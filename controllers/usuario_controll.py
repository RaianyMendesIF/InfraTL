...
class UsuarioController:
    @staticmethod
    async def gerar_hash_ip(ip: str) -> str:
        # Gera hash SHA-256 do IP (LGPD compliance)
        return hashlib.sha256(ip.encode()).hexdigest()

    @staticmethod  
    async def cadastrar_usuario(dados: Usuario_schema_cadastro, session: Session):
        try:
    ...
    @staticmethod  
    async def solicitar_recuperar_senha(dados: Usuario_recuperar_senha, session: Session):
        try:
            usuario = session.query(Usuario).filter(Usuario.email == dados.email).first()
            if usuario:
    ...
    @staticmethod
    async def login_usuario(dados: Login_schema, session: Session, request: Request):
        try:
            ip_cliente = request.client.host if request and request.client else "0.0.0.0"
            ip_hash = await UsuarioController.gerar_hash_ip(ip_cliente)
    ...
    @staticmethod   
    async def listar_cidadoes(session: Session):
        try:
            cidadaoes = session.query(Usuario).filter(Usuario.tipo_usuario == "Usuario").all()
            return {
                "success": True,
                "total": len(cidadaoes),
                "cidadaoes": [Usuario_response.model_validate(c).model_dump() for c in cidadaoes]
            }
