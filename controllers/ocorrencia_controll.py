...
class OcorrenciaController:
    ...
    @staticmethod
    def avaliar_ocorrencia(
        id_ocorrencia: int,
        dados: Avaliar_ocorrencia_schema,
        session: Session,
        id_agente: int,
        background_tasks: BackgroundTasks,
    ):
        ocorrencia = (
            session.query(Ocorrencia).filter(Ocorrencia.id == id_ocorrencia).first()
        )
        ...
        if ocorrencia.status != "Em_Analise":
            raise HTTPException(
                status_code=400,
                detail=f"A ocorrencia nao pode ser avaliada pois ja esta em status: {ocorrencia.status}",
            )
        ...
        if dados.aprovado:
            ...
            if not dados.justificativa:
                raise HTTPException(
                    status_code=400,
                    detail="Necessario passar uma justificativa para o arquivamento da ocorrencia",
                )
    ...
    async def listar_ocorrencias(
        session: Session, bairro: str = None, tipo: str = None, status: str = None
    ):
        try:
            ...
            if tipo:
                servico = (
                    ...
                    .filter(Servico.nome.ilike(f"%{tipo}%"))
                    .first()
                )
                if not servico:
                    raise HTTPException(
                        404, f"Tipo de serviço '{tipo}' não encontrado."
                    )
            ...
            if bairro:
                bairro_obj = (
                    session.query(Bairro)
                    .filter(Bairro.nome.ilike(f"%{bairro}%"))
                    .first()
                )
                if not bairro_obj:
            ...
            return {
                ...
                "ocorrencias": [
                    Ocorrencia_schema_resposta.model_validate(o).model_dump()
